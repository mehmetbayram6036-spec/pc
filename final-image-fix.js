...
productData.forEach((row, index) => {
  ...
  // Resim yolunu web formatına çevir - ID ile eşleştir
  ...
  if (imagePath) {
    ...
    if (firstImagePath.includes('\\')) {
      ...
      const fileName = firstImagePath.split('\\').pop(); // Son dosya adını al
      if (fileName) {
...
productData.slice(0, 5).forEach((row, index) => {
  ...
  const imagePath = row[4] || `/images/${id}.jpg`;
  const firstImagePath = String(imagePath).split(',')[0].trim();
  console.log(`  ${id}. ${name} (${brand}) - ${price} TL - Resim: ${firstImagePath}`);
});
